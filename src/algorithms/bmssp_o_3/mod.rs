...
impl Default for BmsspO3 {
    fn default() -> Self {
        Self::new()
    }
}

impl PathFinder for BmsspO3 {
    ...
    fn shortest_paths(&self, graph: &Graph<(), f64>, source: NodeIndex) -> PathResult {
        ...
        for i in 0..n {
            let rd = &state.relax_data[i];
            if rd.dist < dist_value::safe_infinity() {
                distances.insert(NodeIndex::new(i), rd.dist);
                if i != source.index() && rd.pred != u32::MAX {
                    predecessors.insert(NodeIndex::new(i), NodeIndex::new(rd.pred as usize));
                }
            }
        }
...
mod tests {
    ...
    fn test_with_transformation() {
        let graph = make_complete(10);
        let bmssp = BmsspO3::with_transformation(2);
        let dijkstra = Dijkstra::new();
        let source = NodeIndex::new(0);
