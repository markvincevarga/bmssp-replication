...
impl BmsspBase {
    pub fn new() -> Self {
        Self {
            // off by default: vertex expansion on dense graphs costs more than it saves.
            use_transform: false,
            max_degree: 4,
        }
    }
...
impl Default for BmsspBase {
    fn default() -> Self {
        Self::new()
    }
}

impl PathFinder for BmsspBase {
    ...
    fn shortest_paths(&self, graph: &Graph<(), f64>, source: NodeIndex) -> PathResult {
        ...
        for i in 0..n {
            let d = state.d_hat[i];
            if d < dist_value::safe_infinity() {
                distances.insert(NodeIndex::new(i), d);
                if i != source.index() && state.pred[i] != u32::MAX {
...
mod tests {
    ...
    fn test_with_transformation() {
        let graph = make_complete(10);
        let bmssp = BmsspBase::with_transformation(2);
        let dijkstra = Dijkstra::new();
        let source = NodeIndex::new(0);
