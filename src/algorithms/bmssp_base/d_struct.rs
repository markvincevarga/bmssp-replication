...
thread_local! {
    static PULL_COUNT: Cell<u64> = const { Cell::new(0) };
    static ITEMS_WALKED: Cell<u64> = const { Cell::new(0) };
    static ITEMS_RETURNED: Cell<u64> = const { Cell::new(0) };
    static TIE_RESOLUTION_ITERS: Cell<u64> = const { Cell::new(0) };
}
...
pub struct PullStats {
    ...
    pub tie_resolution_iters: u64,
}

pub fn reset_pull_stats() {
    PULL_COUNT.with(|c| c.set(0));
    ITEMS_WALKED.with(|c| c.set(0));
    ITEMS_RETURNED.with(|c| c.set(0));
    TIE_RESOLUTION_ITERS.with(|c| c.set(0));
}

pub fn read_pull_stats() -> PullStats {
    PullStats {
        pulls: PULL_COUNT.with(|c| c.get()),
        items_walked: ITEMS_WALKED.with(|c| c.get()),
        items_returned: ITEMS_RETURNED.with(|c| c.get()),
        tie_resolution_iters: TIE_RESOLUTION_ITERS.with(|c| c.get()),
    }
}
...
impl DStruct {
    ...
    pub fn pull(&mut self) -> (f64, Vec<u32>) {
        ...
        PULL_COUNT.with(|c| c.set(c.get() + 1));

        let mut s_prime: Vec<DistValue> = Vec::new();
        let mut d0_visited = 0usize;
        while d0_visited < self.d0.len() {
            let id = self.d0[d0_visited];
            let block_len = self.block(id).items.len();
            ITEMS_WALKED.with(|c| c.set(c.get() + block_len as u64));
        ...
        loop {
            ...
            TIE_RESOLUTION_ITERS.with(|c| c.set(c.get() + 1));
        ...
        ITEMS_RETURNED.with(|c| c.set(c.get() + to_return.len() as u64));
        let _ = d1_keys_iter;
